...
local function isFeatureEnabled()
    return getgenv().TeleportIsland or
           getgenv().AutoFarm or
...
local function toggleNoclip(enabled)
    ...
    if enabled and not bodyClip then
        local noclip = Instance.new("BodyVelocity")
        noclip.Name = "BodyClip"
        noclip.Parent = HumanoidRootPart
        noclip.MaxForce = Vector3.new(100000, 100000, 100000)
        noclip.Velocity = Vector3.new(0, 0, 0)
    elseif not enabled and bodyClip then
        bodyClip:Destroy()
    end
end

-- Main loop with optimized delay
spawn(function()
    while true do
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            toggleNoclip(isFeatureEnabled())
        end
        task.wait(0.1) -- Reduced frequency to every 0.1 seconds
    end
end)

-- Handle character reset or respawn
